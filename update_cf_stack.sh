...
CF_BUCKET_URL="https://s3.amazonaws.com/bootstrap-awss3cloudformationbucket-igkja54vcica"
...
ParameterKey=VpcName,ParameterValue="synapsedw-vpc" \
ParameterKey=VpcSubnetPrefix,ParameterValue="10.12""
message=$($UPDATE_CMD 2>&1 1>/dev/null)
error_code=$(echo $?)
if [[ $error_code -ne 0 && $message =~ .*"No updates are to be performed".* ]]; then
  echo -e "\nNo stack changes detected. An update is not required."
...
--template-url $AWS_INFRA_CF_BUCKET_URL/aws-infra/master/$CF_TEMPLATE \
--parameters \
ParameterKey=PeeringConnectionId,ParameterValue="pcx-4904f521" \
ParameterKey=VpcPrivateRouteTable,ParameterValue="rtb-3d932341" \
ParameterKey=VpcPublicRouteTable,ParameterValue="rtb-c49d2db8" \
